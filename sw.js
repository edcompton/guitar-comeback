...
var CACHE_VERSION = 'gchq-v1';
var PRECACHE = ['.', 'index.html', 'data.json', 'manifest.webmanifest', 'icons/icon-192.png', 'icons/icon-512.png'];

self.addEventListener('install', function (e) {
  e.waitUntil(caches.open(CACHE_VERSION).then(function (c) { return c.addAll(PRECACHE); }).then(function () { return self.skipWaiting(); }));
});
...
self.addEventListener('fetch', function (e) {
  ...
  var networkFirst = /(?:^|\/)(data\.json|index\.html)?$/.test(url.pathname);
  if (networkFirst) {
    e.respondWith(
